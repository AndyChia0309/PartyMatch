...
- **群組生命週期狀態機**：招募、額滿、完成服務設定資訊、啟用、確認期、續訂或結束，各階段可執行的操作皆由狀態決定，確保流程不會跳過必要步驟
- **多種共享模式**：依服務類型支援 Email 邀請、地址驗證、Google / Apple 家庭共享、邀請碼與共用帳密，讓不同訂閱服務能接在同一條群組流程上
- **PM 幣代管機制**：申請當下即先行扣款代管，資金並非直接轉入團主帳戶，須待團主啟用服務、成員完成確認後才會撥款；退出、移除、解散或申訴等情境，亦各自對應相應的退款規則
- **信用分數系統**：依使用者行為即時增減，可作為群組申請門檻，並設有重新申請冷卻期
- **可選評價機制**：服務確認後可自行決定是否評價合作對象，之後也能回來補評或更新評價，評分回饋至信用分數並累積合作紀錄
...
- **群組清單資料新鮮度防呆**：探索、收藏、推薦群組等清單畫面僅在進入當下同步一次，使用者長時間閒置分頁後可能看到過期的招募狀態；點擊群組卡片時一律重新查證該群組最新狀態，非團主／成員／申請中者若目標群組已不開放或額滿即攔截並提示，不會誤開已失效的內容，並提供一鍵重新整理；分頁重新取得焦點時另以節流機制靜默刷新整份清單，縮小顯示與實際狀態的落差
...
- [共享模式](docs/product/sharing-modes.md) — 不同服務類型需要的帳號、邀請、家庭共享或共用帳密資訊
